...
namespace ShaneSpace.ProjectedDynamicLinq
{
    ...
    internal class ExpressionParser<TDest>
    {
        struct Token
        {
            public TokenId id;
        ...
        char ch;
        Token token;
        readonly IConfigurationProvider _mapperConfiguration;
        ...
        {
            symbols.Add(name, symbols.Any() ? symbols.First().Value : it);
        }
...
#pragma warning restore 0219
        ...
        Expression ParseMemberAccess(Type type, Expression instance)
        {
            ...
            var mappingConfig = _mapperConfiguration.FindTypeMapFor(type, destinationType);
            if (mappingConfig != null)
            {
                ...
                {
                    // get property map
                    var propMap = propMaps.First(x => x.DestinationProperty.Name == id);

                    // if there is no source member use expression
                    if (propMap.SourceMember == null)
                    {
                        if (propMap.CustomExpression != null)
                        {
                            var body = propMap.CustomExpression.Body;
                            var map = new[] { it }
                                .Select((f, i) => new { f, s = propMap.CustomExpression.Parameters[i] })
                                .ToDictionary(p => p.s, p => p.f);
                            var replaceParameters = ParameterRebinder.ReplaceParameters(map, body);
                            return replaceParameters;
                        }
                    }

                    // Get propery mapped sourcemember name
                    member = FindPropertyOrField(type, propMap.SourceMember.Name, instance == null);
                    if (member != null)
                    {
            ...
            // no mappings found, use standard mapping
            member = FindPropertyOrField(type, id, instance == null);
            if (member != null)
            {
                var memberExpression = member is PropertyInfo
                                                        ? Expression.Property(instance, (PropertyInfo)member)
                                                        : Expression.Field(instance, (FieldInfo)member);
                Debug.WriteLine(memberExpression);
